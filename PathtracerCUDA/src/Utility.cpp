...
uint32_t Utility::findFirstSetBit(uint32_t mask)
{
	uint32_t r = 1;
...
uint32_t Utility::findLastSetBit(uint32_t mask)
{
	uint32_t r = 32;
...
bool Utility::isPowerOfTwo(uint32_t value)
{
	return (value & (value - 1)) == 0 && value > 0;
}

uint32_t Utility::nextPowerOfTwo(uint32_t value)
{
	assert(value > 0);
...
float Utility::halton(size_t index, size_t base)
{
	float f = 1.0f;
	float r = 0.0f;
...
glm::vec3 Utility::colorTemperatureToColor(float kelvin)
{
